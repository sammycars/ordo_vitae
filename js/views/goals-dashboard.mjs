...
class GoalsDashboard {
    ...
    showGoals() {
        ...
        if (currentQuarter) {
            const start = currentQuarter[SCHEMA.QUARTER.columns.start_date] || '';
            const end = currentQuarter[SCHEMA.QUARTER.columns.end_date] || '';
            quarterTitle = `${quarterName}  ${fmt(start)} \u2013 ${fmt(end)}`;
        } else {
            quarterTitle = quarterName;
        }
...
window.GoalsDashboard = GoalsView;
